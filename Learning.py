...
new_data = pd.read_csv("Dataset/predictions.csv")

# Подготовка данных для модели
features = new_data.drop("Failure_90days", axis=1)
target = new_data["Failure_90days"]
...
for model in models:
    ...
    mask_above_1000 = cf_matrix > 500
    mask_below_1000 = cf_matrix <= 500
    ...
    ax1.set_xticklabels(["Legit", "Phishy"], fontsize=12)
    ax1.tick_params(axis='x', labelsize=28)
    ax1.set_ylabel('Actual', fontsize=28, rotation=0, labelpad=50)
    ax1.set_yticklabels(["Legit", "Phishy"], fontsize=12)
    ax1.tick_params(axis='y', labelsize=28)
    ax1.set_title(f'{type(model).__name__}', fontsize=28)
    ...
    plt.savefig(f'ExtractedFeatures/{type(model).__name__}_confusion_matrix.png')
    plt.close()
