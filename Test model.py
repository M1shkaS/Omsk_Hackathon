...
def predict_failures(new_data_path, model_name):
    ...
    features = new_data.drop(["Failure_7days", "Failure_30days", "Failure_90days"], axis=1)
    # Загружаем модель
    ...
    predicted_df = pd.DataFrame(predictions, columns=["Failure_7days", "Failure_30days", "Failure_90days"])

    # Вычисляем метрики
    accuracy = accuracy_score(new_data[["Failure_7days", "Failure_30days", "Failure_90days"]], predicted_df)
    precision = precision_score(new_data[["Failure_7days", "Failure_30days", "Failure_90days"]], predicted_df,
                                average='macro')
    recall = recall_score(new_data[["Failure_7days", "Failure_30days", "Failure_90days"]], predicted_df,
                          average='macro')
